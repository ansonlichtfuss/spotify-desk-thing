...
const NowPlayingStateContext =
	createContext<NowPlayingContextType["state"]>(defaultState);
...
export const NowPlayingContextProvider: ParentComponent = (props) => {
	...
	// 	const response = await setSpotifyShuffle(
	// 		accessToken,
	// 		!state.nowPlaying?.shuffle_state,
	// 	);
	// 	setTimeout(() => nowPlayingQuery.refetch(), 100);
	// };
	...
	const setNowPlayingProgressMs = (callbackMs: (current: number) => number) => {
		setState((current) => ({
			nowPlayingProgressMs: callbackMs(current.nowPlayingProgressMs),
		}));
	};

	return (
		<NowPlayingStateContext value={state}>
			<NowPlayingActionsContext
				value={{
					nowPlayingQuery,
					// setPause,
					// setPlay,
		...
		</NowPlayingStateContext>
	);
};

export const useNowPlayingStateContext = () =>
	useContext(NowPlayingStateContext);
export const useNowPlayingActionsContext = () =>
	useContext(NowPlayingActionsContext);
